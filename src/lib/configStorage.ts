...
const convertSupabaseToLocal = (supabaseConfig: ChatConfiguration): ChatConfig => {
  return {
    ...
    productPrompt: supabaseConfig.product_prompt || 'Cuando el cliente pregunte sobre productos específicos, busca en el catálogo y proporciona información detallada incluyendo precio, descripción y disponibilidad.',
    supportPrompt: supabaseConfig.support_prompt || 'Para consultas técnicas o problemas, consulta la documentación disponible y proporciona soluciones paso a paso.',
    salesPrompt: supabaseConfig.sales_prompt || 'Para ayudar con ventas, destaca las características principales de los productos y guía al cliente hacia la compra de manera natural.',
    styleInstructions: supabaseConfig.style_instructions || 'Usa un español neutro y profesional. Mantén las respuestas concisas pero informativas.',
...
const convertLocalToSupabase = (localConfig: ChatConfig): Partial<ChatConfiguration> => {
  return {
    ...
    product_prompt: localConfig.productPrompt,
    support_prompt: localConfig.supportPrompt,
    sales_prompt: localConfig.salesPrompt,
    style_instructions: localConfig.styleInstructions,
